...
class TestEncryption(TestCase):
    ...
    def test_expand(self):
        ...
        assert (
            key1
            == b"^\xc0+\x91\xa4\xb5\x9coY\xdd_\xbeL\xa6I\xec\xe4\xfa\x85h\xcd\xb8\xba6\xcfABn\x88\x05R+"
        ), key1
        assert len(key1) * 8 == 256, len(key1)
        assert (
            key2
            == b"\xa4\xe2\xae\xac\x19\xa4\x82\x15\x01\xcf`\x91&\xab\x01\xdf%f\x10\x83\xbff\xf9^R\x17\xfe\xe3\x19\x85\x04\xb1"
        ), key2
        ...
        for l in range(0, 40):
            ...
            for i in range(l):
                ...
                assert abs(mean - 1) < 3.3 / sqrt(
                    n
                ), "length %d sum %d for %d samples, norm to %f" % (l, sum[i], n, mean)
